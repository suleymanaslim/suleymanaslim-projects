class WorkoutTimer {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('addMinute').addEventListener('click', () => this.addMinuteToRest());
    }
    ...
    checkSavedTimerState() {
        ...
        if (savedState) {
            ...
            if (state.isRunning) {
                this.workoutType = state.workoutType;
                this.startTime = new Date(state.startTime);
                this.isRunning = true;
                
                // Eğer önceden duraklatılmışsa, aynı durumda devam et
                if (state.isPaused) {
                    this.isPaused = true;
                    this.totalPauseDuration = parseInt(state.totalPauseDuration) || 0;
                    this.lastElapsedTime = state.lastElapsedTime;
                } else {
                    // Duraklatılmamışsa, çıkış-giriş süresini hesapla
                    this.isPaused = true;
                    const lastSaveTime = new Date(state.lastSaveTime).getTime();
                    const timeSinceLastSave = Date.now() - lastSaveTime;
                    this.totalPauseDuration = (parseInt(state.totalPauseDuration) || 0);
                    this.lastElapsedTime = state.lastElapsedTime + timeSinceLastSave;
                }

                // Antrenman modunu aktifleştir
                this.activateWorkoutMode();
                
                // Display'i hemen güncelle
                this.updateDisplay();
            }

            // Eğer aktif bir mola varsa
            if (state.restTimer) {
                this.timeLeft = Math.max(0, state.restTimer.endTime - Date.now()) / 1000;
                if (this.timeLeft > 0) {
                    this.startRestTimer(Math.ceil(this.timeLeft));
                }
            }
        }
    }
    ...
    activateWorkoutMode() {
        ...
        // Antrenman tipini görsel olarak seç
        document.querySelectorAll('.workout-type-btn').forEach(btn => {
            if (btn.dataset.type === this.workoutType) {
        ...
        const pauseButton = document.getElementById('pauseWorkout');
    ...
    saveTimerState() {
        const state = {
            ...
            lastElapsedTime: this.lastElapsedTime,
    ...
    startWorkout() {
        ...
        this.saveTimerState(); // Timer durumunu kaydet
    }
    
    pauseWorkout() {
        this.isPaused = true;
        this.pauseStartTime = Date.now(); // Milisaniye cinsinden tam zamanı kaydet
    ...
    resumeWorkout() {
        if (this.pauseStartTime) {
            // Duraklatma süresini milisaniye cinsinden hesapla
    ...
    stopWorkout() {
        ...
        this.isRunning = false;
        this.isPaused = false;
        localStorage.removeItem('workoutTimerState'); // Timer durumunu temizle
    ...
    updateDisplay() {
        ...
        if (this.isPaused) {
            // Duraklatılmış durumda, kaydedilen son süreyi kullan
            elapsedTime = this.lastElapsedTime;
        } else {
            // Normal çalışma durumunda süreyi hesapla
            const now = Date.now();
            const startTimeMs = this.startTime.getTime();
            elapsedTime = now - startTimeMs - this.totalPauseDuration;
            // Çalışan süreyi kaydet
    ...
    startRestTimer(duration) {
        ...
        this.timeLeft = duration; // Local değişken yerine class property kullan
        ...
        this.restTimer = setInterval(() => {
            this.timeLeft--; // Class property'sini güncelle
            updateRestDisplay();
    ...
    addMinuteToRest() {
        ...
        const minutes = Math.floor(this.timeLeft / 60);
        const seconds = this.timeLeft % 60;
        document.getElementById('restTimeLeft').textContent = 
            `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
        this.saveTimerState(); // Timer durumunu kaydet
    }
    ...
    showNextExercise(index) {
        ...
        form.addEventListener('submit', (e) => {
            e.preventDefault();
            if (this.saveExerciseData(exercise, index)) {
    ...
    saveExerciseData(exercise, index) {
        ...
        // Eğer tükeniş işaretlenmemişse ve tekrar sayısı girilmemişse
        if (!toFailure && !reps) {
            Swal.fire({
                title: 'Hata!',
                text: 'Lütfen tekrar sayısı girin veya tükeniş olarak işaretleyin.',
                icon: 'error',
        ...
        if (exercise.isSuperset) {
            ...
            if (hasWeightDrop) {
                ...
                sets = Array(totalSets).fill().map(() => ({
                    weight: weight,
                    reps: toFailure ? null : parseInt(reps),
                    toFailure: toFailure
                }));
            }
    ...
    saveWorkout() {
        try {
            ...
            const savedWorkouts = JSON.parse(localStorage.getItem('workouts') || '[]');
            savedWorkouts.push(this.currentWorkout);
