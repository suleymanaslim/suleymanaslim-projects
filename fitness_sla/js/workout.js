...
export class WorkoutTimer {
    constructor() {
        ...
        this.restTimer = null;
        this.timeLeft = 0; // Rest timer için süre değişkeni
        this.workouts = {
            bacak: [
    ...
    showNextExercise(index) {
        const exercise = this.workouts[this.workoutType][index];
        if (!exercise) {
            this.finishWorkout();
            return;
        }
                ...
                <div class="modal-content exercise-modal">
                    ...
                    <form id="exerciseForm" class="exercise-form">
                        ...
                        <button type="submit" class="next-exercise-btn">
                            ${index === this.workouts[this.workoutType].length - 1 ? 'Antrenmanı Bitir' : 'Sonraki Hareket'}
                        </button>
                    </form>
                </div>
            </div>
        ...
        exerciseForm.addEventListener('submit', async (e) => {
            ...
            const exerciseData = {
                ...
                sets: parseInt(formData.get('totalSets')),
                isFailure: formData.get('toFailure') === 'on',
            ...
            if (index === this.workouts[this.workoutType].length - 1) {
                this.finishWorkout();
            } else {
    ...
    finishWorkout() {
        ...
        this.showSuccess('Tebrikler!', 'Antrenman başarıyla tamamlandı.');
    }
    ...
    saveExerciseData(exerciseData) {
        ...
        // Firebase'e kaydet
        this.firebaseManager.saveExercise(exerciseData);
    }

    loadWorkouts() {
        try {
            // Önce localStorage'ı temizle (test için)
            ...
            container.innerHTML = '';
            console.log('Loading workouts:', savedWorkouts);
