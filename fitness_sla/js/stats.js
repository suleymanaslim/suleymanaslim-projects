class StatsManager {
    ...
    updateExerciseStats(workouts) {
        ...
        container.innerHTML = Object.values(exerciseStats).map(stats => {
            // Ortalama tekrar hesabı (sadece tükeniş olmayan setler için)
            const avgReps = stats.totalReps > 0 ? 
                (stats.totalReps / stats.totalSets).toFixed(1) : 'N/A';
            
            // Ağırlık düşüşü oranı
            const dropSetRate = ((stats.dropSetCount / stats.totalWorkouts) * 100).toFixed(1);
            ...
            <div class="exercise-progress-card">
                ...
                <div class="exercise-stats">
                    ...
                    <div class="stat-item">
                        <div class="stat-value">${dropSetRate}%</div>
                        <div class="stat-label">Ağırlık Düşüşü</div>
                    </div>
                </div>
                <div class="progress-bar">
                    <div class="progress" style="width: ${dropSetRate}%"></div>
                </div>
            </div>`;
        }).join('');
    }
            ...
            workout.exercises.forEach(exercise => {
                if (!stats[exercise.name]) {
                    stats[exercise.name] = {
                        ...
                        totalSets: 0,
                        totalReps: 0,
                        dropSetCount: 0, // Ağırlık düşüşü sayısı
                        totalWorkouts: 0
                    };
                }
                ...
                // Ağırlık düşüşü kontrolü
                const hasDropSet = exercise.sets.some((set, index, array) => {
                    if (index === 0) return false;
                    return set.weight < array[index - 1].weight;
                });
                
                if (hasDropSet) {
                    exerciseStats.dropSetCount++;
                }

                exercise.sets.forEach(set => {
                    exerciseStats.totalSets++;
                    // Sadece tükeniş olmayan setlerin tekrarlarını say
                    if (!set.toFailure && set.reps) {
