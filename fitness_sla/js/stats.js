...
export class StatsManager {
    ...
    calculateStats() {
        ...
        const recentExercises = exerciseHistory.filter(exercise => 
            new Date(exercise.date) > thirtyDaysAgo
        );
        ...
        recentExercises.forEach(exercise => {
            ...
            stats.totalReps += exercise.reps;
            stats.maxWeight = Math.max(stats.maxWeight, exercise.weight);
            if (exercise.isFailure) stats.failureCount++;
            stats.totalEntries++;
        });

        Object.entries(exerciseStats).forEach(([exerciseName, stats]) => {
            const failureRate = ((stats.failureCount / stats.totalEntries) * 100).toFixed(1);
            ...
            if (card) {
                ...
                statsContainer.innerHTML += `
                    ...
                    <div class="stat-item">
                        <div class="stat-label">Toplam Tükeniş</div>
